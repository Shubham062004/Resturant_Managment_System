...
export const getInventoryRequests = async (
  ...
  try {
    ...
    const requests = await InventoryService.getInventoryRequests(
      branchId,
      status
    );
...
export const approveInventoryRequest = async (
  ...
  try {
    const request = await InventoryService.approveInventoryRequest(
      req.params.id,
      req.body
    );
...
export const updateInventoryRequestStatus = async (
  ...
  try {
    ...
    const request = await InventoryService.updateInventoryRequestStatus(
      req.params.id,
      status
    );
    res.status(200).json({ status: 'success', data: request });
