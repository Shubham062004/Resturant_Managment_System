...
async function main() {
  ...
  console.log(
    `Found ${staff.length} staff members and ${branches.length} branches. Generating HR & Ops History...`,
  );
    ...
    for (const employee of staff) {
      ...
      if (!isAbsent) {
        ...
        workAssignmentInserts.push({
          ...
          roleAssigned:
            employee.role === Role.KITCHEN_STAFF
              ? faker.helpers.arrayElement(['Pizza Station', 'Burger Station', 'Prep Line'])
              : employee.role,
          date: currentDate,
        });
      } else {
