...
export default function StaffManagementPage() {
  ...
  const totalPayroll = staffList.reduce((sum: number, s: any) => sum + (parseFloat(s.salary) || 0), 0);
  const avgPerformance = staffList.length > 0
    ? (staffList.reduce((sum: number, s: any) => sum + (s.performanceScore || 5.0), 0) / staffList.length).toFixed(1)
    : '5.0';
    ...
    <div className="space-y-8 p-6 text-[#F8FAFC] bg-[#0F172A] min-h-screen font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Configure compensation matrices, monitor shift logs, and manage organizational category roles.</p>
        </div>
        <Button className="bg-[#2563EB] hover:bg-[#2563EB]/95 text-white shadow-lg">Add Staff Member</Button>
      </div>

      {/* KPI Stats Row */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <Card className="p-6 bg-[#111827] border-slate-800 shadow-lg">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Active Headcount</p>
        ...
        <Card className="p-6 bg-[#111827] border-slate-800 shadow-lg">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Avg Efficiency Score</p>
      ...
      {alertMsg && (
        <div className={`p-4 mb-4 rounded-xl border flex justify-between items-center text-sm ${
          alertMsg.type === 'success' 
            ? 'bg-emerald-950/20 border-emerald-800/30 text-emerald-400' 
            : 'bg-rose-950/20 border-rose-800/30 text-rose-400'
        }`}>
          <span>{alertMsg.text}</span>
          <button onClick={() => setAlertMsg(null)} className="text-slate-400 hover:text-white font-bold ml-2">✕</button>
        </div>
      )}
          ...
          {selectedIds.length > 0 && (
            <Card className="p-4 bg-indigo-950/20 border border-[#2563EB]/30 flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <Button className="bg-[#2563EB] hover:bg-[#2563EB]/95 text-white" size="sm" onClick={handleBulkUpdate}>
                  Apply Actions
                </Button>
                <Button className="bg-slate-800 border-slate-700 text-slate-300 hover:bg-slate-750" size="sm" onClick={() => setSelectedIds([])}>
                  Cancel
                </Button>
              </div>
            </Card>
                  ...
                  {staffList.map((staff: any) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="p-4">
                          <div>
                            <div className="text-white font-semibold">{staff.firstName} {staff.lastName}</div>
                            <div className="text-slate-500 text-xs mt-0.5">{staff.email}</div>
                          </div>
                        </td>
                        <td className="p-4">
                          <div className="space-y-1">
                            <Badge variant="info" className="text-xs uppercase bg-[#2563EB]/10 text-white border border-[#2563EB]/20">
                        ...
                        <td className="p-4">
                          <span
                            className={`px-2 py-1 text-xs rounded-full font-bold ${
                              staff.isActive ? 'bg-[#16A34A]/10 text-[#16A34A] border border-[#16A34A]/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'
                            }`}
                          >
            ...
            <div className="space-y-6">
              ...
              <Card className="p-5 border border-slate-800 bg-[#111827] shadow-lg space-y-4 text-white">
                <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                  ...
                  <Badge variant={editIsActive ? 'success' : 'error'} className={editIsActive ? 'bg-[#16A34A]/10 text-[#16A34A]' : 'bg-red-500/10 text-red-400'}>
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-400 uppercase mb-1">Current Staff Role</label>
                    <select
                      id="edit-staff-role"
                      value={editRole}
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-400 uppercase mb-1">Category Station Filter</label>
                    <select
                  ...
                  <div className="flex items-center justify-between pt-2">
                    <span className="text-sm text-slate-300 font-medium">Activate Profile Status</span>
                    <button
              ...
              <Card className="p-5 border border-slate-800 bg-[#111827] shadow-lg space-y-4 text-white">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-400 uppercase mb-1">Manual Shift Count Override</label>
                    <Input
                  ...
                  <div className="bg-slate-950 p-3 rounded border border-slate-850 text-xs text-slate-500 space-y-1">
                    <div className="font-semibold text-slate-450">June 2026 Virtual Shift Sheet:</div>
                    <div className="grid grid-cols-7 gap-1 text-center mt-2">
                      {Array.from({ length: 14 }).map((_, idx) => (
                        <div
                          key={idx}
                          className={`p-1 rounded text-[10px] font-bold ${
                            idx < editAttendance ? 'bg-[#16A34A]/20 text-[#16A34A] border border-[#16A34A]/20' : 'bg-slate-900 text-slate-650 border border-slate-800/40'
                          }`}
                        >
                          D{idx + 1}
                        </div>
              ...
              <Card className="p-5 border border-slate-800 bg-[#111827] shadow-lg space-y-4 text-white">
                <h3 className="text-base font-bold text-white border-b border-slate-800 pb-3">Compensation Matrix & Payout</h3>
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-1">
                      <label className="block text-xs font-semibold text-slate-400 uppercase">Performance Rating (1.0 - 5.0)</label>
                      <span className="text-xs font-bold text-amber-500 font-mono">{editPerformance} Stars</span>
                    </div>
                    <input
                      type="range"
                      min="1.0"
                ...
                <div className="pt-2">
                  <Button className="w-full bg-[#2563EB] hover:bg-[#2563EB]/90 text-white font-semibold" onClick={handleSaveProfile}>
                    Commit & Sync Changes
                  </Button>
                </div>
              </Card>
