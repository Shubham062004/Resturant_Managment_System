...
export default function AdminLayout() {
  ...
  const isOwner = role === 'ORGANIZATION_OWNER' || role === 'FRANCHISE_OWNER' || role === 'SUPER_ADMIN';
  ...
  const sidebarGroups: SidebarGroup[] = isOwner ? [
    {
      ...
      links: [
        ...
        { name: 'Ingredient Management', path: '/admin/ingredients', icon: <ClipboardList size={18} /> },
        ...
        { name: 'Table Reservations', path: '/admin/reservations', icon: <Calendar size={18} /> }
      ]
    },
    {
      name: 'Management',
    ...
    <div className="flex h-screen bg-[#0F172A] text-[#F8FAFC] overflow-hidden font-sans">
        ...
        {!isCollapsed && (
          <div className="px-4 py-3.5 border-b border-slate-800/60 space-y-3 shrink-0">
            ...
            <div className="space-y-1">
              <label className="text-[10px] uppercase font-bold text-slate-500 tracking-wider">Active Franchise</label>
              <select
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <div className="md:hidden bg-[#111827] border-t border-slate-800/80 h-14 shrink-0 flex items-center justify-around z-20">
          <Link to="/admin" className="flex flex-col items-center gap-0.5 text-slate-400 hover:text-[#2563EB] transition-colors">
            <Home size={18} />
            <span className="text-[9px] font-bold">Home</span>
          </Link>
          <Link to="/admin/orders" className="flex flex-col items-center gap-0.5 text-slate-400 hover:text-[#2563EB] transition-colors">
            <Box size={18} />
            <span className="text-[9px] font-bold">Orders</span>
          </Link>
          <Link to="/admin/kitchen" className="flex flex-col items-center gap-0.5 text-slate-400 hover:text-[#2563EB] transition-colors">
            <Activity size={18} />
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {sidebarGroups.map((group) => (
                  <div key={group.name} className="space-y-1">
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block px-2.5 mb-1.5">{group.name}</span>
              ...
              <div className="p-4 border-t border-slate-800 bg-slate-900/40 flex justify-between items-center">
                ...
                <button onClick={handleLogout} className="text-slate-450 hover:text-red-400 p-1.5 hover:bg-slate-850 rounded-lg">
                  <LogOut size={16} />
                </button>
              </div>
            </motion.div>
