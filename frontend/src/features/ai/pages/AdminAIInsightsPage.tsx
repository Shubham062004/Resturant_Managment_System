...
export default function AdminAIInsightsPage() {
  ...
  const { demand, inventory, status } = useAppSelector(
  ...
  const triggerForecast = () => {
    ...
    dispatch(
      fetchPredictions({ branchId: selectedBranchId, type: 'inventory' })
    );
  ...
  useEffect(() => {
    if (selectedBranchId) {
      dispatch(
        fetchPredictions({ branchId: selectedBranchId, type: 'demand' })
      );
      dispatch(
        fetchPredictions({ branchId: selectedBranchId, type: 'inventory' })
      );
  ...
  const totalDemandOrders =
    demand?.forecasts?.reduce(
      (acc: number, f: any) => acc + f.expectedOrders,
      0
    ) || 0;
  const avgOrdersExpected =
    demand?.forecasts?.length > 0
      ? Math.round(totalDemandOrders / demand.forecasts.length)
      : 0;
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1 ml-9">
            Leverage neural demand forecasting, low stockout preventative
            analytics, and custom branch optimization recommendations.
          </p>
        </div>
            ...
            {status === 'loading' ? (
              <div className="flex flex-col items-center justify-center py-16 text-slate-500">
                ...
                <p className="text-xs font-semibold">
                  Generating neural demand charts...
                </p>
              </div>
            ) : demand ? (
              <>
          ...
          <CardContent className="p-0 space-y-5">
            {status === 'loading' ? (
              <div className="flex flex-col items-center justify-center py-16 text-slate-500">
                ...
                <p className="text-xs font-semibold">
                  Running supply depletion analysis...
                </p>
              </div>
            ) : inventory ? (
              <>
                ...
                <div className="space-y-3">
                  ...
                  {(!inventory.risks || inventory.risks.length === 0) && (
                    <div className="p-8 text-center bg-slate-950/30 rounded-2xl border border-slate-850">
                      <CheckCircle
                        size={24}
                        className="text-emerald-450 mx-auto mb-2"
                      />
      ...
      <Card className="bg-[#111827] border-slate-800 shadow-xl p-6">
        <CardHeader className="p-0 border-none mb-4">
          <h3 className="text-base font-bold font-display text-white flex items-center gap-2">
            <Sparkles size={16} className="text-[#06B6D4]" /> AI Business
            Strategy & Decisions Advisory
          </h3>
          <p className="text-xs text-slate-400">
            Prescriptive chain-wide strategy suggestions synthesized from
            consumer retention splits and branch ratings
          </p>
        </CardHeader>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 font-sans text-xs">
          <div className="p-4 bg-slate-950/40 border border-slate-850 rounded-2xl space-y-2">
            ...
            <p className="font-bold text-white text-sm">
              Target Peak Hour Surge Promos
            </p>
            <p className="text-[10px] text-slate-450 leading-relaxed">
              Adjust Friday/Saturday checkout coupons dynamic rates during 7-9
              PM. System predicts +8.2% gross revenue lift.
            </p>
          </div>
          <div className="p-4 bg-slate-950/40 border border-slate-850 rounded-2xl space-y-2">
            ...
            <p className="font-bold text-white text-sm">
              Optimize Delivery Roster Shift
            </p>
            <p className="text-[10px] text-slate-450 leading-relaxed">
              Allocate additional delivery drivers to central branch zone on
              Sundays starting 5 PM to mitigate standard queue delays.
            </p>
          </div>
          <div className="p-4 bg-slate-950/40 border border-slate-850 rounded-2xl space-y-2">
            ...
            <p className="font-bold text-white text-sm">
              Expand Featured Veg Varieties
            </p>
            <p className="text-[10px] text-slate-450 leading-relaxed">
              Vegetarian category items search volumes increased by 14.5%
              month-over-month. Promote top rating veg dishes in landing cards.
            </p>
          </div>
        </div>
      </Card>
