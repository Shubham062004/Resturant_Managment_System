...
export default function ProductMetrics() {
  ...
  const { topProducts, categories } = product;
        ...
        <CardContent className="h-80 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={topProducts} layout="vertical" margin={{ top: 5, right: 30, left: 40, bottom: 5 }}>
              ...
              <YAxis dataKey="productName" type="category" stroke="#ffffff60" fontSize={12} tickLine={false} axisLine={false} width={100} />
              <Tooltip 
            ...
            {topProducts.map((p: any, i: number) => (
              <div key={i} className="flex justify-between items-center p-3 bg-secondary/50 rounded-lg">
                ...
                <div className="text-right">
                  ...
                  <Badge variant="success" className="mt-1">${p.revenue.toFixed(2)}</Badge>
                </div>
              </div>
            ))}
          </div>
