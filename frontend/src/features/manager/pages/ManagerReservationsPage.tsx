...
import { fetchBranchReservations, updateReservationStatus } from '../../reservations/store/reservationSlice';
...
export default function ManagerReservationsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'CANCELLED': return 'bg-rose-500/20 text-rose-500 border-rose-500/30';
  ...
  }).sort((a, b) => new Date(`${a.reservationDate}T${a.reservationTime}`).getTime() - new Date(`${b.reservationDate}T${b.reservationTime}`).getTime());
  ...
  return (
    <div className="flex flex-col h-full space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 shrink-0">
        <div>
          <h1 className="text-3xl font-bold font-display text-white tracking-tight">Reservations Desk</h1>
          <p className="text-sm text-slate-400 mt-1">Manage incoming table bookings and guest check-ins.</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pb-6">
        {resStatus === 'loading' ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 bg-slate-900/40 rounded-2xl border border-border/20 text-slate-500">
            ...
            <p className="text-sm font-semibold">No reservations found for {dateFilter.toLowerCase()}.</p>
          </div>
        ) : (
              ...
              {filteredReservations.map((res: any, idx: number) => (
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <Badge className={`text-[10px] font-bold px-2 py-0.5 border ${getStatusBadge(res.status)}`}>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4 p-3 bg-slate-950/50 rounded-xl border border-border/10">
                    <div>
                      <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1">Time & Date</p>
                      <p className="text-sm text-slate-200 flex items-center gap-1.5 font-medium">
                        <Clock size={14} className="text-primary" />
                        {res.reservationTime} <span className="text-xs text-slate-500 ml-1">({res.reservationDate})</span>
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1">Party Size</p>
                      <p className="text-sm text-slate-200 flex items-center gap-1.5 font-medium">
                  ...
                  <div className="mt-auto pt-4 border-t border-border/10 flex flex-wrap gap-2">
                    {res.status === 'PENDING' && (
                      <>
                        <Button onClick={() => handleUpdateStatus(res.id, 'CONFIRMED')} size="sm" className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white py-1.5 text-xs font-bold">
                          <CheckCircle size={14} className="inline mr-1" /> Confirm
                        </Button>
                        <Button onClick={() => handleUpdateStatus(res.id, 'CANCELLED')} size="sm" className="flex-1 bg-rose-500/20 text-rose-500 border border-rose-500/30 hover:bg-rose-500/30 py-1.5 text-xs font-bold">
                    ...
                    {res.status === 'CONFIRMED' && (
                      <>
                        <Button onClick={() => handleUpdateStatus(res.id, 'CHECKED_IN')} size="sm" className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-1.5 text-xs font-bold">
                          Check In Guest
                        </Button>
                        <Button onClick={() => handleUpdateStatus(res.id, 'NO_SHOW')} size="sm" className="bg-slate-800 text-slate-400 hover:text-white border border-border/20 py-1.5 text-xs font-bold">
                          No Show
                        </Button>
                      </>
                    )}
