  ...
  return (
    <div className="flex flex-col h-full space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 shrink-0">
        <div>
          <h1 className="text-3xl font-bold font-display text-white tracking-tight">Customer Relations</h1>
          <p className="text-sm text-slate-400 mt-1">Manage local branch customers, feedback, and issue resolution.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 min-h-0">
        ...
        <Card className="lg:col-span-1 bg-slate-900/40 border-border/20 flex flex-col h-full overflow-hidden">
          <div className="p-4 border-b border-border/10 bg-slate-900/60 flex items-center justify-between shrink-0">
            ...
            <Badge className="bg-indigo-500/20 text-indigo-400 font-bold">{filteredCustomers.length}</Badge>
          </div>
          
          <div className="flex-1 overflow-y-auto p-3 custom-scrollbar space-y-2">
            {loading ? (
              <div className="space-y-2">
                {[1, 2, 3].map(i => <div key={i} className="h-16 bg-slate-900/50 animate-pulse rounded-xl" />)}
              </div>
              ...
              filteredCustomers.map((customer) => (
                ...
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="text-sm font-bold text-slate-200">{customer.firstName} {customer.lastName}</p>
                      <p className="text-[10px] text-slate-500 mt-0.5">{customer.email || 'No email'}</p>
                    </div>
                  ...
                  <div className="flex items-center gap-4 text-xs font-semibold text-slate-400">
                    <span className="flex items-center gap-1"><ShoppingBag size={12} className="text-emerald-500" /> {customer.orderCount} Orders</span>
              ...
              <div className="p-6 border-b border-border/20 bg-slate-900/80 backdrop-blur-md shrink-0 flex justify-between items-start">
                <div className="flex gap-4 items-center">
                  <div className="w-16 h-16 rounded-2xl bg-indigo-500/20 flex items-center justify-center border border-indigo-500/30">
                    <span className="text-2xl font-bold text-indigo-400">{selectedCustomer.firstName[0]}</span>
                  </div>
                  <div>
                    <h2 className="text-2xl font-bold font-display text-white">
                      {selectedCustomer.firstName} {selectedCustomer.lastName}
                    </h2>
                    <p className="text-sm text-slate-400 mt-1">{selectedCustomer.email} • {selectedCustomer.phone || 'No Phone'}</p>
                  </div>
                </div>
                <div className="flex flex-col items-end gap-2">
                  <Badge variant={selectedCustomer.totalSpent > 5000 ? 'warning' : 'neutral'} className="text-xs font-bold px-3 py-1">
                    {selectedCustomer.totalSpent > 5000 ? 'VIP Customer' : 'Standard'}
                  </Badge>
                  <Button size="sm" variant="outline" className="h-8 text-xs border-indigo-500/30 text-indigo-400 hover:bg-indigo-500/10">
              ...
              <div className="flex-1 overflow-y-auto p-6 custom-scrollbar space-y-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="p-4 bg-slate-950/50 rounded-xl border border-border/10">
                    ...
                    <p className="text-2xl font-bold text-slate-200">{selectedCustomer.orderCount}</p>
                  </div>
                  <div className="p-4 bg-slate-950/50 rounded-xl border border-border/10">
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Lifetime Value</p>
                    <p className="text-2xl font-bold text-emerald-400 font-mono">₹{selectedCustomer.totalSpent.toLocaleString()}</p>
                  </div>
                  <div className="p-4 bg-slate-950/50 rounded-xl border border-border/10">
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Avg Order Val</p>
                    <p className="text-2xl font-bold text-slate-200 font-mono">₹{Math.round(selectedCustomer.totalSpent / selectedCustomer.orderCount).toLocaleString()}</p>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {selectedCustomer.feedback.map((fb: any, idx: number) => (
                        <div key={idx} className="p-3 bg-rose-500/5 rounded-xl border border-rose-500/20">
                          ...
                          <div className="flex justify-between items-center mt-2">
                            ...
                            <Badge className={`text-[10px] ${fb.resolved ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}`}>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {selectedCustomer.recentOrders.sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(0, 5).map((order: any) => (
                      <div key={order.id} className="p-4 bg-slate-950/60 rounded-xl border border-border/10 flex justify-between items-center group hover:border-slate-700 transition-colors">
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="font-bold font-mono text-slate-300">#{order.orderNumber.slice(-6).toUpperCase()}</span>
                            <Badge className="text-[9px] bg-slate-800 text-slate-400 border-none px-1.5 py-0.5">{order.orderType}</Badge>
                          </div>
                          <p className="text-xs text-slate-500 mt-1">{new Date(order.createdAt).toLocaleString()}</p>
                        </div>
                        <div className="text-right">
                          <p className="font-bold text-emerald-400 font-mono">₹{order.totalAmount.toLocaleString()}</p>
