...
export default function ManagerStaffPage() {
  ...
  const getRoleBadge = (role: string) => {
    ...
    if (role.includes('KITCHEN') || role.includes('CHEF')) return 'bg-amber-500/20 text-amber-500 border-amber-500/30';
  ...
  return (
    <div className="flex flex-col h-full space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 shrink-0">
        <div>
          <h1 className="text-3xl font-bold font-display text-white tracking-tight">Staff Roster</h1>
          <p className="text-sm text-slate-400 mt-1">Manage branch employees, track attendance and performance.</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pb-6">
        {status === 'loading' ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {[1, 2, 3, 4].map(i => <div key={i} className="h-40 bg-slate-900/50 animate-pulse rounded-2xl border border-border/10" />)}
          </div>
              ...
              {filteredStaff.map((staff: any, idx: number) => (
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center overflow-hidden shrink-0">
                        <span className="text-sm font-bold text-slate-300">{staff.firstName[0]}</span>
                      </div>
                      <div>
                        <h3 className="text-sm font-bold text-white tracking-wide truncate max-w-[120px]">
                          {staff.firstName} {staff.lastName}
                        </h3>
                        <Badge className={`mt-1 text-[9px] px-1.5 py-0 border ${getRoleBadge(staff.role)}`}>
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-4 p-3 bg-slate-950/50 rounded-xl border border-border/10">
                    <div>
                      <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-0.5">Attendance</p>
                    ...
                    <div>
                      <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-0.5">Rating</p>
                      <p className="text-sm text-slate-200 flex items-center gap-1 font-mono">
                  ...
                  <div className="mt-auto pt-3 border-t border-border/10 flex items-center justify-between">
                    ...
                    <Button size="sm" variant="outline" className="text-[10px] h-7 px-3 border-border/20 bg-slate-900 hover:bg-slate-800 text-slate-300">
                      View Profile
                    </Button>
                  </div>
                </motion.div>
