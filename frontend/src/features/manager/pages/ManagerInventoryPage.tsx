...
import { fetchInventory, fetchInventoryRequests, createInventoryRequest } from '../../inventory/store/inventorySlice';
...
export default function ManagerInventoryPage() {
  ...
  const [requestItems, setRequestItems] = useState<{ ingredientId: string, quantity: number, notes: string }[]>([]);
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'REJECTED': return 'bg-rose-500/20 text-rose-500 border-rose-500/30';
    ...
    <div className="flex flex-col h-full space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 shrink-0">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Monitor branch stock and order supplies from Central.</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button onClick={() => setShowRequestForm(true)} className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold h-9">
        ...
        <div className="xl:col-span-1 flex flex-col gap-6 h-full overflow-y-auto custom-scrollbar">
              ...
              {lowStockItems.length === 0 ? (
                ...
                lowStockItems.slice(0, 5).map((item, i) => (
                  <div key={i} className="flex justify-between items-center p-3 bg-amber-500/5 rounded-xl border border-amber-500/10">
                    <div>
                      <p className="text-sm font-semibold text-amber-500">{item.ingredient?.name || item.name}</p>
                      <p className="text-[10px] text-amber-500/70 mt-0.5">Min: {item.minimumStockLevel} {item.unit}</p>
                    </div>
                    <span className="font-mono text-amber-500 font-bold">{item.quantity} {item.unit}</span>
                  </div>
                ))
              )}
            </div>
          ...
          <Card className="bg-slate-900/40 border-border/20 flex flex-col flex-1 overflow-hidden min-h-[300px]">
            <div className="p-4 border-b border-border/10 bg-slate-900/60 flex items-center justify-between shrink-0">
              ...
              <Badge className="bg-slate-800 text-slate-300 font-bold">{branchInventory.length} Items</Badge>
            </div>
            <div className="flex-1 overflow-y-auto p-4 custom-scrollbar space-y-2">
              {branchInventory.map((item, i) => (
                <div key={i} className="flex justify-between items-center p-3 bg-slate-950/50 rounded-xl border border-border/5">
                  <div>
                    <p className="text-sm font-semibold text-slate-200">{item.ingredient?.name || item.name}</p>
                    <p className="text-[10px] text-slate-500 uppercase tracking-widest mt-0.5">{item.unit}</p>
                  </div>
                  <span className={`font-mono font-bold ${item.quantity <= (item.minimumStockLevel || 10) ? 'text-amber-500' : 'text-slate-300'}`}>
                    {item.quantity}
                  </span>
                </div>
              ))}
            ...
            {inventoryRequests.length === 0 ? (
              ...
              <div className="space-y-4">
                {inventoryRequests.map(req => (
                  <div key={req.id} className="p-4 bg-slate-950/60 rounded-2xl border border-border/10 flex flex-col gap-3 group hover:border-border/30 transition-colors">
                    <div className="flex justify-between items-start">
                      <div>
                        <h4 className="font-bold text-slate-200">Request #{req.requestNumber?.toUpperCase() || req.id.slice(-6).toUpperCase()}</h4>
                    ...
                    <div className="pt-3 border-t border-border/5 grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Items Requested</p>
                        {req.items?.slice(0, 3).map((item: any, idx: number) => (
                          <p key={idx} className="text-xs text-slate-300 flex justify-between">
                            ...
                            <span className="font-mono text-primary">{item.quantity} {item.ingredient?.unit}</span>
                          </p>
                        ))}
                        {(req.items?.length || 0) > 3 && (
                          <p className="text-[10px] text-slate-500 italic">+ {(req.items?.length || 0) - 3} more items</p>
                        )}
                      </div>
                      <div className="space-y-3">
                        <div>
                          <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1">Expected Delivery</p>
                          <p className="text-xs text-slate-300">{new Date(req.expectedDate).toLocaleDateString()}</p>
                        </div>
      ...
      <AnimatePresence>
        {showRequestForm && (
          <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-slate-900 border border-border/20 max-w-2xl w-full rounded-2xl flex flex-col max-h-[80vh] shadow-2xl">
              <div className="p-6 border-b border-border/20 flex justify-between items-center shrink-0">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-1">Order ingredients from the central warehouse.</p>
                </div>
                <button onClick={() => setShowRequestForm(false)} className="text-slate-500 hover:text-white">✕</button>
              </div>
              
              <div className="p-6 flex-1 overflow-y-auto custom-scrollbar space-y-4">
                {requestItems.map((item, idx) => (
                  <div key={idx} className="flex flex-col sm:flex-row gap-3 p-4 bg-slate-950/50 rounded-xl border border-border/10">
                    <div className="flex-1">
                      <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1 block">Ingredient</label>
                      <select 
                    ...
                    <div className="flex-1">
                      <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1 block">Notes (Opt)</label>
                      <Input 
                        type="text" value={item.notes} placeholder="Urgent, specific brand..."
                        onChange={(e) => handleUpdateItem(idx, 'notes', e.target.value)}
                        className="bg-slate-900 border-border/30"
                      />
                    </div>
                    <div className="flex items-end pb-1">
                      <button onClick={() => handleRemoveRequestItem(idx)} className="text-rose-500 hover:text-rose-400 p-1.5 bg-rose-500/10 rounded-lg">
                        ✕
                      </button>
                    </div>
                  </div>
                ...
                <Button onClick={handleAddRequestItem} variant="outline" className="w-full border-dashed border-border/40 text-slate-400 hover:text-white hover:border-primary/50 bg-transparent h-12">
                  + Add Ingredient to Request
                </Button>
              </div>
              
              <div className="p-6 border-t border-border/20 flex gap-3 justify-end bg-slate-900/50 shrink-0">
                <Button variant="outline" onClick={() => setShowRequestForm(false)} className="border-border/30 text-slate-300 hover:bg-slate-800">Cancel</Button>
                <Button onClick={handleSubmitRequest} disabled={submitting || requestItems.length === 0} className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold">
