...
export default function ManagerTablesPage() {
  ...
  const getTableColor = (status: string) => {
    switch (status) {
      ...
      case 'RESERVED':
        return 'bg-amber-500/20 border-amber-500 text-amber-400 hover:bg-amber-500/30';
      case 'CLEANING':
        return 'bg-sky-500/20 border-sky-500 text-sky-400 hover:bg-sky-500/30';
      default:
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 shrink-0">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Drag and drop to arrange. Click to manage assignments.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            onClick={() => dispatch(fetchTables(selectedBranchId))}
            ...
            {filteredTables.map((table) => (
              ...
              >
                <span className="font-display tracking-widest">
                  {table.number.replace('T-', '')}
                </span>
                <span className="text-[10px] font-normal mt-0.5 opacity-80 flex items-center gap-1">
        ...
        <div className="xl:col-span-1 bg-slate-900/40 rounded-2xl border border-border/20 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-3 custom-scrollbar">
            {filteredTables.map((table) => (
              <Card
                key={`list-${table.id}`}
                className="bg-slate-950/60 border-border/10 p-3 flex flex-col gap-3"
              >
                ...
                <div className="flex flex-wrap gap-2 pt-2 border-t border-border/5">
                  {table.status === 'AVAILABLE' && (
                    <Button
                      ...
                      size="sm"
                      className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 text-[10px] h-7"
                    >
                      Seat Guest
                    </Button>
                  )}
                  {table.status === 'OCCUPIED' && (
                    <Button
                      ...
                      size="sm"
                      className="flex-1 bg-sky-600/20 text-sky-400 hover:bg-sky-600/30 border border-sky-500/30 text-[10px] h-7"
                    >
                      Mark Cleaning
                    </Button>
                  )}
                  {table.status === 'CLEANING' && (
                    <Button
                      ...
                      size="sm"
                      className="flex-1 bg-emerald-600/20 text-emerald-400 hover:bg-emerald-600/30 border border-emerald-500/30 text-[10px] h-7"
                    >
                      Cleaned & Ready
                    </Button>
                  )}
