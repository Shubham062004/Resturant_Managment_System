...
import { fetchActiveOrders, receiveNewOrder, receiveOrderStatusUpdate } from '../../kitchen/store/kitchenSlice';
...
export default function ManagerKitchenPage() {
  ...
  useEffect(() => {
    ...
    socket.on('kds_status_update', (kitchenOrder: any) => dispatch(receiveOrderStatusUpdate(kitchenOrder)));
  ...
  const delayedOrders = branchOrders.filter(o => getElapsedTime(o.createdAt) > 20 && o.status !== 'PACKED');
  ...
  return (
    <div className="flex flex-col h-full space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 shrink-0">
        <div>
          <h1 className="text-3xl font-bold font-display text-white tracking-tight">Kitchen Load & KDS</h1>
          <p className="text-sm text-slate-400 mt-1">Monitor station queues, prep times, and ticket delays.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 shrink-0">
        ...
        <Card className="p-4 bg-slate-900/60 border-border/20 flex flex-col justify-center">
          ...
          <span className="text-2xl font-bold text-white">18<span className="text-sm font-normal text-slate-500 ml-1">mins</span></span>
        </Card>
      </div>
      ...
      <div className="flex-1 overflow-x-auto pb-4">
        <div className="flex gap-4 h-full min-w-[900px]">
          ...
          {[
            { id: 'QUEUED', title: 'Pending', items: queued, color: 'text-amber-500', bg: 'bg-amber-500/10' },
            { id: 'COOKING', title: 'Cooking', items: cooking, color: 'text-orange-500', bg: 'bg-orange-500/10' },
            { id: 'READY', title: 'Ready / QA', items: ready, color: 'text-emerald-500', bg: 'bg-emerald-500/10' },
          ].map((col) => (
            <div key={col.id} className="flex-1 flex flex-col bg-slate-900/30 rounded-2xl border border-border/10 overflow-hidden">
              <div className={`p-4 border-b border-border/20 flex items-center justify-between shrink-0 bg-slate-900/80 backdrop-blur-md`}>
                <div className="flex items-center gap-3">
                      ...
                      return (
                        ...
                        >
                          <div className="flex justify-between items-start">
                            <div>
                              <span className="font-bold text-slate-200">#{order.order?.orderNumber?.slice(-6).toUpperCase() || 'TKT'}</span>
                              <span className="text-[10px] text-slate-400 block mt-0.5">
                                {order.order?.orderType}
                              </span>
                            </div>
                            <Badge className={`text-[10px] flex items-center gap-1 ${isDelayed ? 'bg-rose-500/20 text-rose-400 border-none' : 'bg-slate-800 text-slate-400 border-none'}`}>
                          ...
                          <div className="pt-2 border-t border-border/10 space-y-1.5">
                            {order.tasks?.map((task: any) => (
                              <div key={task.id} className="flex justify-between text-xs font-medium text-slate-300">
