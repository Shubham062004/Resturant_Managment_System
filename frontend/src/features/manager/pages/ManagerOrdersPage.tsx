    ...
    return (
      ...
      >
        <div className="flex justify-between items-start">
          ...
          <Badge
            variant={order.orderType === 'DELIVERY' ? 'warning' : 'info'}
        ...
        <div className="mt-2 pt-3 border-t border-border/10 flex flex-wrap gap-2">
          {order.status === 'PLACED' && (
            <>
              <Button
                ...
                size="sm"
                className="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-[10px] py-1 h-7"
              >
                Accept Order
              </Button>
              <Button
                ...
                size="sm"
                className="bg-slate-800 hover:bg-rose-900/50 text-slate-400 hover:text-rose-400 text-[10px] py-1 h-7 border border-border/20"
              >
                <XCircle size={14} />
              </Button>
            </>
          ...
          {['ACCEPTED'].includes(order.status) && (
            <Button
              ...
              size="sm"
              className="flex-1 bg-amber-600 hover:bg-amber-700 text-white text-[10px] py-1 h-7"
            >
              Send to Kitchen
            </Button>
          )}
          {['PREPARING'].includes(order.status) && (
            <Button
              ...
              size="sm"
              className="flex-1 bg-purple-600 hover:bg-purple-700 text-white text-[10px] py-1 h-7"
            >
              Mark Ready
            </Button>
          )}
          ...
          <Button
            size="sm"
            className="bg-slate-800 hover:bg-slate-700 text-slate-300 text-[10px] py-1 h-7 px-2 border border-border/20"
          >
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 shrink-0">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Manage kitchen dispatch and fulfillment workflow.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            onClick={fetchOrders}
            className="bg-slate-800 hover:bg-slate-700 text-white border border-border/30 rounded-xl h-9"
          >
            Refresh
          </Button>
        </div>
          ...
          {columns.map((col) => {
            ...
            return (
              ...
              >
                {/* Column Header */}
                <div
                  className={`p-4 border-b border-border/20 flex items-center justify-between shrink-0 bg-slate-900/80 backdrop-blur-md sticky top-0 z-10`}
                >
                  <div className="flex items-center gap-3">
                    <div
                  ...
                  {loading ? (
                    <div className="animate-pulse space-y-4">
                      {[1, 2].map((i) => (
                        <div
                          key={i}
                          className="h-32 bg-slate-800/50 rounded-xl border border-border/10"
                        />
                      ))}
                    </div>
