...
export const fetchAssignedOrders = createAsyncThunk('delivery/fetchAssignedOrders', async () => {
  const response = await api.get('/delivery/orders');
  return response.data.data.orders;
});

export const acceptOrder = createAsyncThunk('delivery/acceptOrder', async (orderId: string) => {
  const response = await api.patch(`/delivery/orders/${orderId}/accept`);
  return response.data.data.order;
});

export const pickupOrder = createAsyncThunk('delivery/pickupOrder', async (orderId: string) => {
...
export const updateLocation = createAsyncThunk('delivery/updateLocation', async (location: any) => {
...
const deliverySlice = createSlice({
  ...
  initialState,
  reducers: {
    socketAssignmentUpdate: (state, action: PayloadAction<DeliveryAssignment>) => {
...
export const { socketAssignmentUpdate, setActiveAssignment } = deliverySlice.actions;
