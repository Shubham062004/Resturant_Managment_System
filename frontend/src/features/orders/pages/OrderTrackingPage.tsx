...
export default function OrderTrackingPage() {
  ...
  if (status === 'loading') return <MainLayout><div className="p-8 text-white">Loading...</div></MainLayout>;
  if (!currentOrder) return <MainLayout><div className="p-8 text-white">Order not found.</div></MainLayout>;
    ...
    <MainLayout>
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        <div className="flex justify-between items-center">
          <h1 className="text-3xl font-display font-bold text-white">Tracking Order #{currentOrder.orderNumber}</h1>
        ...
        <Card className="bg-surface/50 border-border/50 p-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
            <div>
              <h2 className="text-xl font-bold text-white mb-2">Order Status: <span className="text-primary">{displayStatus}</span></h2>
              <p className="text-muted-foreground text-sm">Placed on {new Date(currentOrder.createdAt).toLocaleString()}</p>
            </div>
          ...
          {!isCancelled ? (
            <div className="relative">
              ...
              <div className="space-y-6 md:space-y-0 md:flex md:justify-between relative">
                {timelineSteps.map((step, index) => {
                  ...
                  return (
                    <div key={step.status} className="flex flex-row md:flex-col items-center gap-4 md:gap-2 z-10 relative">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center border-2 ${
                        isCompleted ? 'bg-primary border-primary text-white' : 'bg-surface border-border/50 text-muted-foreground'
                      } ${isActive ? 'ring-4 ring-primary/30' : ''}`}>
                        <Icon size={20} />
                      </div>
                      <div className="md:text-center">
                        <p className={`font-semibold text-sm ${isCompleted ? 'text-white' : 'text-muted-foreground'}`}>{step.label}</p>
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="p-6 bg-danger/10 border border-danger/20 rounded-xl flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-muted-foreground text-sm">Your order has been cancelled and a refund has been initiated.</p>
              </div>
            </div>
          )}
        </Card>
