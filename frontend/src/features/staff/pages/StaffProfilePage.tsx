  ...
  return (
    <div className="flex flex-col space-y-6 max-w-5xl mx-auto h-full overflow-y-auto custom-scrollbar pb-10">
      ...
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">Manage your personal information and view earnings.</p>
      </div>
          ...
          <Card className="p-6 bg-slate-900/60 border-border/20 text-center flex flex-col items-center">
            <div className="w-24 h-24 rounded-full bg-slate-800 border-4 border-slate-900 shadow-xl flex items-center justify-center overflow-hidden mb-4 relative group">
              {user?.avatar ? (
                ...
                <span className="text-3xl font-bold text-slate-400">{user?.firstName?.[0] || 'S'}</span>
              )}
            </div>
            ...
            <div className="w-full mt-6 pt-6 border-t border-border/10 space-y-3">
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-500 flex items-center gap-2"><Briefcase size={14} /> Employee ID</span>
                <span className="font-mono text-slate-300 font-bold">EMP-{user?.id?.slice(0, 6).toUpperCase() || '000000'}</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-500 flex items-center gap-2"><MapPin size={14} /> Branch</span>
          ...
          <Card className="p-6 bg-slate-900/60 border-border/20">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3 p-3 bg-slate-950/50 rounded-xl border border-border/5">
                <div className="p-2 bg-slate-900 rounded-lg"><Phone size={16} className="text-slate-400" /></div>
                <div>
                  <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest">Phone Number</p>
                  <p className="text-sm text-slate-300 font-medium">{user?.phone || '+91 98765 43210'}</p>
                </div>
              </div>
              <div className="flex items-center gap-3 p-3 bg-slate-950/50 rounded-xl border border-border/5">
                <div className="p-2 bg-slate-900 rounded-lg"><Mail size={16} className="text-slate-400" /></div>
                <div>
          ...
          <Card className="p-6 bg-slate-900/40 border-border/20">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-4 bg-slate-950/50 rounded-xl border border-border/10">
                <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest mb-1">Department</p>
                <p className="font-bold text-slate-200">Kitchen Operations</p>
              </div>
              <div className="p-4 bg-slate-950/50 rounded-xl border border-border/10">
                <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest mb-1">Designation</p>
                <p className="font-bold text-slate-200">Senior Line Cook</p>
              </div>
              <div className="p-4 bg-slate-950/50 rounded-xl border border-border/10">
                <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest mb-1">Joining Date</p>
                <p className="font-bold text-slate-200">{new Date(user?.createdAt || Date.now()).toLocaleDateString()}</p>
              </div>
              <div className="p-4 bg-slate-950/50 rounded-xl border border-border/10">
                <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest mb-1">Manager</p>
          ...
          <Card className="p-6 bg-slate-900/40 border-border/20 overflow-hidden relative group">
            ...
            <div className="space-y-4 relative">
              <div className="flex items-center justify-between p-4 bg-slate-950/50 rounded-xl border border-border/10 transition-all">
                ...
                <span className={`text-xl font-mono ${showEarnings ? 'text-white' : 'text-slate-500 tracking-widest'}`}>
              ...
              <div className="flex items-center justify-between p-4 bg-slate-950/50 rounded-xl border border-border/10 transition-all">
                ...
                <span className={`text-xl font-mono ${showEarnings ? 'text-amber-400' : 'text-slate-500 tracking-widest'}`}>
              ...
              <div className="flex items-center justify-between p-4 bg-slate-950/50 rounded-xl border border-border/10 transition-all">
                ...
                <span className={`text-xl font-mono ${showEarnings ? 'text-sky-400' : 'text-slate-500 tracking-widest'}`}>
              ...
              <div className="flex items-center justify-between p-5 bg-gradient-to-r from-emerald-500/10 to-transparent rounded-xl border border-emerald-500/20 mt-4">
                <span className="font-bold text-white uppercase tracking-widest text-sm">Total Earnings</span>
                <span className={`text-3xl font-bold font-mono ${showEarnings ? 'text-emerald-400' : 'text-emerald-500/50 tracking-widest'}`}>
