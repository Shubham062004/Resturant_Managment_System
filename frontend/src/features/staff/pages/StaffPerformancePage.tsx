...
export default function StaffPerformancePage() {
  ...
  return (
    <div className="flex flex-col space-y-6 h-full overflow-y-auto custom-scrollbar pb-10 max-w-6xl mx-auto w-full">
      ...
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold font-display text-white tracking-tight">Performance & Rewards</h1>
        <p className="text-sm text-slate-400 mt-1">Track your efficiency, customer ratings, and bonus earnings.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="p-5 bg-slate-900/60 border-border/20 backdrop-blur-xl">
          ...
          <h3 className="text-2xl font-bold font-mono text-white mb-1">{metrics.todayCompleted} <span className="text-sm font-sans font-normal text-slate-500">items</span></h3>
        ...
        <Card className="p-5 bg-slate-900/60 border-border/20 backdrop-blur-xl">
          <div className="flex justify-between items-start mb-2">
            ...
            {metrics.avgPrepTime < metrics.targetPrepTime && (
              <Badge className="bg-emerald-500/20 text-emerald-400 border-none text-[10px] px-1.5 py-0">Fast</Badge>
            )}
          </div>
          <h3 className="text-2xl font-bold font-mono text-white mb-1">{metrics.avgPrepTime}</h3>
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Avg Prep Time</p>
        </Card>
        ...
        <Card className="p-5 bg-slate-900/60 border-border/20 backdrop-blur-xl">
          ...
          <h3 className="text-2xl font-bold font-mono text-white mb-1">{metrics.customerRating} <span className="text-sm font-sans font-normal text-slate-500">/ 5.0</span></h3>
        ...
        <Card className="p-5 bg-slate-900/60 border-border/20 backdrop-blur-xl">
          ...
          <h3 className="text-2xl font-bold font-mono text-white mb-1">#{metrics.leaderboardRank} <span className="text-sm font-sans font-normal text-slate-500">/ {metrics.totalStaff}</span></h3>
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Branch Rank</p>
        </Card>
      </div>
          ...
          <div className="relative space-y-4">
            <div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-amber-500/70 mt-1">Earn ₹{bonusData.bonusPerRating} for every perfect rating!</p>
              </div>
              <span className="text-3xl font-black text-amber-500">{bonusData.fiveStarRatings}</span>
            </div>

            <div className="grid grid-cols-3 gap-4">
              <div className="p-4 bg-slate-950/50 rounded-xl border border-border/10 text-center">
                <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest mb-1">Daily Bonus</p>
                <p className={`text-xl font-bold font-mono ${showBonus ? 'text-emerald-400' : 'text-slate-500'}`}>
                  {showBonus ? `₹${bonusData.dailyBonus}` : '****'}
                </p>
              </div>
              <div className="p-4 bg-slate-950/50 rounded-xl border border-border/10 text-center">
                <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest mb-1">Weekly Bonus</p>
                <p className={`text-xl font-bold font-mono ${showBonus ? 'text-emerald-400' : 'text-slate-500'}`}>
                  {showBonus ? `₹${bonusData.weeklyBonus}` : '****'}
                </p>
              </div>
              <div className="p-4 bg-slate-950/50 rounded-xl border border-border/10 text-center">
                <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest mb-1">Monthly Bonus</p>
                <p className={`text-xl font-bold font-mono ${showBonus ? 'text-emerald-400' : 'text-slate-500'}`}>
        ...
        <Card className="p-6 bg-slate-900/40 border-border/20">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500 mt-2">Only 2 orders returned or remade this month.</p>
            </div>

            <div>
              <div className="flex justify-between items-end mb-2">
                <span className="text-sm font-semibold text-slate-300">Manager Evaluation</span>
                <span className="text-xs font-bold text-sky-500">{metrics.managerRating} / 5.0</span>
              </div>
              <div className="w-full bg-slate-800 rounded-full h-2">
                <div className="bg-sky-500 h-2 rounded-full" style={{ width: '90%' }}></div>
              </div>
              <p className="text-[10px] text-slate-500 mt-2">"Consistently delivers high quality under pressure."</p>
            </div>
          </div>
        </Card>
