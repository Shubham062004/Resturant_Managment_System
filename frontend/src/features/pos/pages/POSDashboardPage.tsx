...
export default function POSDashboardPage() {
  ...
  const { terminals, activeDrawer, cart, activeOrder, status } = useAppSelector(
    (state) => state.pos,
  );
      ...
      <div className="max-w-md mx-auto mt-20 p-6 bg-surface border border-border/50 rounded-xl space-y-6 animate-fade-in">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mt-2">
            Open the cash drawer to begin accepting orders.
          </p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {cart.items.length === 0 ? (
            ...
            cart.items.map((item: any) => (
              <div
                key={item.productId}
                className="flex items-center justify-between bg-background/50 p-3 rounded-lg border border-border/30"
              >
          ...
          {!activeOrder ? (
            ...
            <div className="space-y-3 animate-fade-in">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <Button
                  className="h-14 bg-blue-600 hover:bg-blue-700 font-bold"
