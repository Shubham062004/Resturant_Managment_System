...
export default function POSDashboardPage() {
  ...
  const { terminals, activeDrawer, cart, activeOrder, receipts, status } =
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      setAlertMsg({
        type: 'success',
        text: 'POS Invoice draft created. Process payment below.',
      });
    } catch (err: any) {
  ...
  const handlePayment = async (method: 'CASH' | 'UPI' | 'CARD') => {
    ...
    if (method === 'UPI' && !upiReference) {
      setAlertMsg({
        type: 'error',
        text: 'Please enter the UPI transaction reference number.',
      });
      return;
    }
    ...
    try {
      ...
      if (
        ...
        result.state?.status === 'PAID'
      ) {
        setAlertMsg({
          type: 'success',
          text: 'Payment completed successfully. Order closed.',
        });
        ...
        setAlertMsg({
          type: 'success',
          text: 'Payment received. Awaiting full completion.',
        });
      }
    } catch (err: any) {
      setAlertMsg({
      ...
      <div className="max-w-md mx-auto mt-20 p-6 bg-slate-900 border border-slate-800 rounded-xl space-y-6 animate-fade-in text-slate-100">
        <div className="text-center">
          ...
          <p className="text-slate-400 mt-2">
            Open the cash drawer terminal registry to begin walk-in register
            checkout operations.
          </p>
        </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            >
              ...
              <option value="demo-terminal-id">
                Main Register #1 (Auto-Fallback)
              </option>
            </select>
          </div>
          <div>
    ...
    <div className="h-[calc(100vh-6rem)] flex flex-col gap-4 p-4 text-slate-100 max-w-7xl mx-auto">
      ...
      <div className="flex justify-between items-center border-b border-slate-800 pb-3">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            REGISTER ACTIVE: #
            {activeDrawer.terminalId?.slice(0, 6) || 'Main Register'}
          </Badge>
        </div>
      </div>
      ...
      {viewTab === 'register' ? (
                ...
                cart.items.map((item: any) => (
                  ...
                  >
                    <div className="flex-1 pr-2">
                      <h4 className="text-slate-200 font-semibold text-xs truncate">
                        {item.name}
                      </h4>
            ...
            <div className="p-4 bg-slate-950/60 border-t border-slate-850 space-y-4">
              ...
              {!activeOrder ? (
                <div className="flex gap-2">
                  <Button
                    variant="outline"
                    className="h-12 border-slate-700 text-slate-300"
                  ...
                  <div className="space-y-2 bg-slate-950 p-3 rounded border border-slate-850">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] uppercase font-extrabold text-slate-400 flex items-center gap-1">
                        <QrCode className="w-3.5 h-3.5 text-indigo-400" /> UPI
                        Pay QR Code
                      </span>
        ...
        <div className="flex-1 overflow-y-auto space-y-4">
          <h2 className="text-lg font-bold text-white flex items-center gap-2">
            <History className="w-5 h-5 text-indigo-400" /> Shift Payout
            Transactions Log
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {receipts.map((rec: any, idx) => (
              <Card
                key={rec.id || idx}
                className="bg-slate-900 border-slate-800 p-4 space-y-3 flex flex-col justify-between"
              >
                <div>
                  <div className="flex justify-between items-center border-b border-slate-850 pb-2">
                    <span className="font-semibold text-white text-sm">
                      Invoice #{rec.id?.slice(0, 8)}
                    </span>
                  ...
                  <div className="text-xs text-slate-400 mt-2 font-mono flex flex-col gap-1">
                    ...
                    <span>
                      ...
                      {parseFloat(
                        rec.order?.totalAmount || rec.total || 0
                      ).toFixed(2)}
                    </span>
          ...
          <Card className="bg-slate-900 border-slate-800 max-w-sm w-full p-6 text-slate-100 flex flex-col space-y-4">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <h3 className="text-sm uppercase font-bold text-slate-400 flex items-center gap-2">
                <Printer className="w-4 h-4 text-indigo-400" /> Thermal Print
                Slip
              </h3>
              <button
            ...
            >
              <div className="text-center space-y-1">
                ...
                <p className="text-[10px] text-slate-600">
                  Branch Register Cashier Receipt
                </p>
                <p className="text-[9px] text-slate-500">
                  June 2026 Shift Ledger
                </p>
              </div>
              ...
              <div className="space-y-1.5 text-[10px]">
                {printedReceipt.items?.map((item: any, i: number) => (
                  <div key={i} className="flex justify-between items-center">
                    ...
                    <div className="font-semibold text-right w-16">
                      ₹
                      {(
              ...
              <div className="border-t border-dashed border-black/35 pt-2 text-[10px] space-y-1">
                <div className="flex justify-between font-bold text-[11px]">
                  ...
                  <span>
                    ...
                    {(
                      parseFloat(
                        printedReceipt.order?.totalAmount ||
