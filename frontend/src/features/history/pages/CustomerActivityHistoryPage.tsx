...
export default function CustomerActivityHistoryPage() {
  ...
  const { data: response, isLoading } = useHistoryQuery('customers', { limit: 500 });
  const records = useMemo(() => response?.data || [], [response]);

  // Derived KPIs
  const stats = useMemo(() => {
    if (!records.length) return { totalCustomers: 0, repeatRate: 0, avgSpend: 0, retention: 0 };
  ...
  return (
    <div className="flex flex-col space-y-6 h-full overflow-y-auto pb-10">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-bold font-display text-white tracking-tight">Customer Analytics</h1>
          <p className="text-sm text-slate-400 mt-1">Retention, purchasing behaviors, and demographic growth.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <StatCard title="Total Customers" value={stats.totalCustomers.toLocaleString()} trend={14.2} icon={Users} loading={isLoading} />
        <StatCard title="Repeat Purchase Rate" value={`${stats.repeatRate.toFixed(1)}%`} trend={4.1} icon={Activity} loading={isLoading} />
        <StatCard title="Average Lifetime Spend" value={formatCurrency(stats.avgSpend)} trend={2.5} icon={Heart} loading={isLoading} />
        <StatCard title="Estimated Retention" value={`${stats.retention.toFixed(1)}%`} trend={-1.2} icon={Calendar} loading={isLoading} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="bg-slate-900/40 border-border/20 lg:col-span-2">
          <CardHeader className="border-b border-border/10 pb-4">
            <h3 className="font-bold text-white text-sm">Customer Growth & Loyalty</h3>
          </CardHeader>
          <CardContent className="p-6 h-[300px]">
            {isLoading ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData.customerGrowth} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  <defs>
                  ...
                  <Area type="monotone" dataKey="repeat" name="Repeat Customers" stroke="#10b981" fillOpacity={1} fill="url(#colorRep)" />
                  <Area type="monotone" dataKey="new" name="New Customers" stroke="#8b5cf6" fillOpacity={1} fill="url(#colorNew)" />
                </AreaChart>
              </ResponsiveContainer>
            )}
          </CardContent>
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-border/10">
                {filteredRecords.length === 0 ? (
                  <tr><td colSpan={5} className="px-6 py-16 text-center text-slate-500">No customers found.</td></tr>
                ) : (
