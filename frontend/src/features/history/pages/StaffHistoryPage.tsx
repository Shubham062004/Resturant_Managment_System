...
export default function StaffHistoryPage() {
  ...
  const filteredRecords = useMemo(() => {
    return records.filter((r: any) => {
      const matchSearch = (r.firstName + ' ' + r.lastName)
        .toLowerCase()
    ...
    <div className="flex flex-col space-y-6 h-full overflow-y-auto pb-10">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Workforce composition, retention, and performance metrics.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <StatCard
          ...
          value={`${stats.attrition.toFixed(1)}%`}
          trend={-1.5}
          icon={UserMinus}
          loading={isLoading}
        />
        ...
        />
        <StatCard
          title="Est. Payroll/Mo"
          value={formatCurrency(stats.payroll)}
          trend={8.4}
          icon={DollarSign}
          loading={isLoading}
        />
          ...
          <CardContent className="p-6 h-[300px]">
            {isLoading ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={chartData.deptDistribution}
                    cx="50%"
                    cy="50%"
                    innerRadius={60}
                    outerRadius={80}
