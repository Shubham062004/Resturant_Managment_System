...
export default function InventoryHistoryPage() {
  ...
  const { data: response, isLoading } = useHistoryQuery('inventory', {
    limit: 500,
  });
  ...
  const stats = useMemo(() => {
    if (!records.length)
      return { requests: 0, movements: 0, waste: 0, value: 0 };
    return {
  ...
  const chartData = useMemo(() => {
    ...
    const branchComparison = Object.keys(branchMap)
      .map((name) => ({
    ...
    <div className="flex flex-col space-y-6 h-full overflow-y-auto pb-10">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Multi-branch stock movements, requests, and waste tracking.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="bg-slate-900/40 border-border/20">
          <CardHeader className="border-b border-border/10 pb-4">
            <h3 className="font-bold text-white text-sm">
              Consumption vs Restock (Monthly)
            </h3>
          </CardHeader>
          <CardContent className="p-6 h-[300px]">
            {isLoading ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart
                  data={chartData.consumptionTrend}
                  margin={{ top: 10, right: 10, left: -20, bottom: 0 }}
                >
                  <CartesianGrid
                    strokeDasharray="3 3"
