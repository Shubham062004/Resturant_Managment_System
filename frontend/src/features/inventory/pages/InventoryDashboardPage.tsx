...
import { Package, AlertTriangle, ArrowRightLeft, Trash2, ShoppingCart, Truck, CheckCircle, Clock } from 'lucide-react';
...
export default function InventoryDashboardPage() {
  ...
  const { inventory, analytics, inventoryRequests, status } = useAppSelector((state) => state.inventory);
  ...
  const [requestItems, setRequestItems] = useState<Array<{ ingredientId: string; requestedQuantity: number }>>([]);
  ...
  const handleCreateRequest = async () => {
    if (requestItems.length === 0) {
      setAlertMsg({ type: 'error', text: 'Please select at least one ingredient to request replenishment.' });
      return;
    }
    ...
    try {
      ...
      setAlertMsg({ type: 'success', text: 'Replenishment request dispatched to central supply hub.' });
  ...
  const handleUpdateStatus = async (id: string, nextStatus: 'PACKED' | 'DISPATCHED' | 'DELIVERED', type: 'dispatch' | 'deliver') => {
    try {
      await dispatch(
        updateInventoryRequestStatus({
          id,
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">
            Real-time stock balancing, central supplier dispatches, and multi-branch replenishment requests.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card className="bg-slate-900 border-slate-800 p-6 flex flex-col justify-center">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-slate-400 font-semibold text-sm uppercase tracking-wider">Total Stock Items</h2>
          </div>
        ...
        <Card className="bg-slate-900 border-slate-800 p-6 flex flex-col justify-center">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-slate-400 font-semibold text-sm uppercase tracking-wider">Low Stock alerts</h2>
          </div>
        ...
        <Card className="bg-slate-900 border-slate-800 p-6 flex flex-col justify-center">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-slate-400 font-semibold text-sm uppercase tracking-wider">Active POs</h2>
          </div>
        ...
        <Card className="bg-slate-900 border-slate-800 p-6 flex flex-col justify-center">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-slate-400 font-semibold text-sm uppercase tracking-wider">Waste Today (Units)</h2>
          </div>
      ...
      <div className="flex border-b border-slate-800 gap-4">
        <button
          ...
          className={`pb-3 text-sm font-semibold tracking-wider uppercase transition border-b-2 ${
            activeTab === 'stock' ? 'border-indigo-500 text-indigo-400 font-bold' : 'border-transparent text-slate-400 hover:text-slate-200'
          }`}
        >
          🗄️ Stock Levels
        </button>
        <button
          ...
          className={`pb-3 text-sm font-semibold tracking-wider uppercase transition border-b-2 ${
            activeTab === 'requests' ? 'border-indigo-500 text-indigo-400 font-bold' : 'border-transparent text-slate-400 hover:text-slate-200'
          }`}
        >
                        ...
                        return (
                          <tr key={inv.id || i} className="hover:bg-slate-900/50 transition-colors">
                            ...
                            <td className="px-6 py-4">
                              {isLow ? (
                                <Badge variant="warning" className="bg-amber-500/10 text-amber-500 border border-amber-500/20">
                                  Low Stock
                                </Badge>
                              ) : (
            ...
            <div className="space-y-4">
              <Card className="bg-slate-900 border-slate-800 p-5 hover:border-indigo-500/40 cursor-pointer transition-colors flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">Procure raw stock elements directly from configured suppliers.</p>
                </div>
              </Card>
              ...
              <Card className="bg-slate-900 border-slate-800 p-5 hover:border-indigo-500/40 cursor-pointer transition-colors flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">Shift units from overflowing branches to low stocks.</p>
                </div>
              </Card>
              ...
              <Card className="bg-slate-900 border-slate-800 p-5 hover:border-indigo-500/40 cursor-pointer transition-colors flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">Account for spoiled, broken, or expired items.</p>
                </div>
              </Card>
            </div>
          </div>
              ...
              return (
                <Card key={req.id} className="bg-slate-900 border-slate-800 p-6 space-y-4">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 pb-4">
                    <div className="space-y-1">
                      ...
                      <div className="text-xs text-slate-400 flex items-center gap-3">
                        <span>Requested By: {req.requestedBy?.firstName || 'Staff'} ({req.requestedBy?.email || 'System'})</span>
                        <span>•</span>
                    ...
                    <div className="flex gap-2">
                      ...
                      {req.status === 'PENDING' && (user?.role === 'ADMIN' || user?.role === 'SUPER_ADMIN' || user?.role === 'ORGANIZATION_OWNER') && (
                        <>
                          <Button variant="primary" size="sm" onClick={() => handleApproveRequest(req.id, 'APPROVED')}>
                            Approve
                          </Button>
                          <Button variant="outline" className="border-slate-700 text-slate-300 hover:bg-slate-800" size="sm" onClick={() => handleApproveRequest(req.id, 'REJECTED')}>
                            Reject
                          </Button>
                        </>
                      )}
                      ...
                      {req.status === 'APPROVED' && (user?.role === 'INVENTORY_MANAGER' || user?.role === 'ADMIN' || user?.role === 'SUPER_ADMIN') && (
                        <Button variant="primary" size="sm" onClick={() => handleUpdateStatus(req.id, 'PACKED', 'dispatch')}>
                          Mark Packed
                        </Button>
                      )}
                      {req.status === 'PACKED' && (user?.role === 'INVENTORY_MANAGER' || user?.role === 'ADMIN' || user?.role === 'SUPER_ADMIN') && (
                        <Button variant="primary" size="sm" onClick={() => handleUpdateStatus(req.id, 'DISPATCHED', 'dispatch')}>
                          Dispatch Cargo
                        </Button>
                      )}

                      {/* Deliver Confirm (Branch Manager, Admin) */}
                      {req.status === 'DISPATCHED' && (user?.role === 'BRANCH_MANAGER' || user?.role === 'ADMIN' || user?.role === 'SUPER_ADMIN') && (
                        <Button variant="success" size="sm" onClick={() => handleUpdateStatus(req.id, 'DELIVERED', 'deliver')}>
                      ...
                      {/* Final Status pill */}
                      {req.status === 'REJECTED' && (
                        <Badge variant="error" className="bg-red-500/10 text-red-500 border border-red-500/20">
                          Rejected
                        </Badge>
                      )}
                    ...
                    <div className="py-2">
                      <div className="flex justify-between items-center relative">
                        ...
                        {[
                          ...
                          return (
                            <div key={step.value} className="flex flex-col items-center z-10 space-y-1 bg-slate-900 px-3">
                              <div
                              ...
                              <span className={`text-[11px] font-semibold tracking-wider ${isCompleted ? 'text-indigo-400 font-bold' : 'text-slate-500'}`}>
                                {step.label}
                              </span>
                            </div>
                          );
                  ...
                  <div className="bg-slate-950/40 rounded-lg p-4 border border-slate-800/60">
                    <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Requested Restock Materials</h4>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {req.items?.map((item: any) => (
                        <div key={item.id} className="flex justify-between items-center p-2.5 bg-slate-950 border border-slate-800 rounded">
                          <span className="text-sm text-slate-200 font-medium">{item.ingredient?.name || 'Ingredient'}</span>
                    ...
                    {req.notes && (
                      <div className="mt-3 text-xs text-slate-500 border-t border-slate-800/80 pt-3">
                        <span className="font-semibold text-slate-400">Restock Notes:</span> {req.notes}
                      </div>
                    )}
                  </div>
                </Card>
            ...
            {inventoryRequests.length === 0 && (
              <div className="p-12 text-center border-2 border-dashed border-slate-800 rounded-lg text-slate-500 flex flex-col items-center justify-center">
                ...
                <p className="text-xs text-slate-500 mt-1 max-w-sm">
                  Create a replenishment request using the button above to request stock transfers from the central warehouse supply.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="fixed inset-0 bg-slate-950/80 flex items-center justify-center p-4 z-50 animate-fade-in backdrop-blur-sm">
          <Card className="bg-slate-900 border-slate-800 p-6 max-w-lg w-full space-y-4 text-slate-100 shadow-2xl">
            ...
            <div className="space-y-4">
              {/* Ingredient select */}
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Select Ingredients</label>
              ...
              {requestItems.length > 0 && (
                <div className="space-y-2 max-h-[180px] overflow-y-auto">
                  ...
                  {requestItems.map((item) => {
                    const ing = inventory.find((i) => i.ingredientId === item.ingredientId)?.ingredient;
                    return (
                      <div key={item.ingredientId} className="flex justify-between items-center gap-4 bg-slate-950 border border-slate-800 p-2 rounded">
                        <span className="text-xs truncate font-medium text-slate-300">{ing?.name}</span>
                        <div className="flex items-center gap-2">
                          <Input
                            id={`request-qty-${item.ingredientId}`}
                            type="number"
                            value={item.requestedQuantity}
                            onChange={(e) => updateItemQty(item.ingredientId, parseFloat(e.target.value) || 0)}
              ...
              {/* Notes */}
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Restock Notes</label>
                <Input
                  id="request-notes"
                  type="text"
