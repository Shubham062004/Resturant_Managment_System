...
export function StatCard({
  ...
  return (
    <Card
      className={`bg-slate-900/60 border-border/10 overflow-hidden relative group ${className || ''}`}
    >
      <CardContent className="p-6 relative z-10">
        ...
        {loading ? (
          ...
          <div className="flex items-baseline space-x-3">
            ...
            {trend !== undefined && (
              <span
                className={`text-sm font-medium px-2 py-0.5 rounded-full ${trend >= 0 ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}
              >
                {trend >= 0 ? '+' : ''}
                {trend}%
              </span>
        ...
        {subtitle && !loading && <p className="text-slate-500 text-xs mt-2">{subtitle}</p>}
      </CardContent>
